...
const ContactInfo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await API.put(`/contactInfoUpdate/${editingId}`, data, { headers: { "Content-Type": "multipart/form-data" } });
        toast.success("Contact info updated");
      } else {
        await API.post("/contactInfoCreate", data, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  return (
    <div className="p-6">
      {/* Form */}
      <form onSubmit={handleSubmit} className="p-4 bg-white shadow rounded mb-6 flex flex-col gap-3">
        <h2 className="text-xl font-semibold">{editingId ? "Edit Contact Info" : "Add Contact Info"}</h2>
        <input type="email" name="email" placeholder="Email" className="border p-2 w-full rounded" value={formData.email} onChange={handleChange} required />
        <input type="text" name="phone" placeholder="Phone" className="border p-2 w-full rounded" value={formData.phone} onChange={handleChange} required />
        ...
        <input type="file" name="resume" className="border p-2 w-full rounded" onChange={handleFileChange} accept=".pdf,.doc,.docx" />
        <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">{editingId ? "Update" : "Save"}</button>
      </form>
